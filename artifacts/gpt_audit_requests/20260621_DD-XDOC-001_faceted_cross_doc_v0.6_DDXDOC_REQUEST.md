---
request_id: DD-XDOC-001-v0.6-20260621
supersedes_request_id: DD-XDOC-001-v0.5-20260621
topic: faceted cross-document comparison & alignment candidate v0.6（round5 must_fix 9点・決定式閉鎖版）の独立意味監査
gate: DDXDOC
status: queued
result_expected_filename: 20260621_DD-XDOC-001_faceted_cross_doc_v0.6_DDXDOC_RESULT.md
target_mode: inline_embedded
source_hash: sha256:3774ef64840b771111c713fc99b90873c450cede8d41cf31c1f9796f63421c05
prior_result_file_id: 2299484641489
review_scope:
  include:
    - B1/must1: symmetric cardinality を unordered side-size から決定し未定義分岐を除去（受入試験9）。side canonicalization と cardinality canonicalization の関数分離。multi-member side が辞書順先頭でも一意か
    - B2/must2: coverage_claim_scope + required_ranges + coverage_complete_for_scope（covered union ⊇ required union AND unknown ∩ required = ∅）。covered_ranges=[] で必ず false か（受入試験6）
    - B3/must3: origin_relation に unknown enum 追加・§8-3 mapping table の null 置換（schema 矛盾解消）
    - B4/must4: priority=110（任意軸 invalid → ineligible）・priority=105（observation stale → hold）で observation 軸の穴を閉じたか
    - B5/must5: computed_evidence_ref typed・computed_input_fingerprint canonicalization・review audit 条件付き必須・G_XDOC_INDEP_ACTIVE_UNIQUE
    - B6/must6: decision_payload + decision_payload_digest 定義・revision uniqueness/one-active/supersedes・status を履歴導出（in-place 更新なし）
    - B7/must7: method_registry_id = sha256(method_id,version)・capability rule が versioned 参照・candidate_only 廃止（emits_candidate_relation + allowed_non_candidate_assertion_types）
    - B8/must8: support_edge_effective predicate・minimum_support_score・density を pairwise_support_coverage に一本化・typed outlier/missing_pair rule
    - must9/P0-8: ellipsis・union（int|string, A[] or B[]）・未定義 digest/fingerprint・origin null の完全除去（受入試験1機械通過）
    - should_fix 5点（cluster size≥2・low≠high・非決定 method result_payload_digest・extraction guard・lineage edge 分離）
    - 受入試験12本が本文だけから決定的に実装可能か（特に 1, 6, 9, 11）
  exclude:
...
regression_anchors:
  - DD-XDOC-001 v0.5 REQUEST（Box 2299271140180）
  - DD-XDOC-001 v0.5 RESULT（Box 2299484641489）label=DDXDOC_MODIFY_REQUIRED
  - DD-XDOC-001 v0.4 RESULT（Box 2299235092570）label=DDXDOC_MODIFY_REQUIRED
  - DD-LAYOUT-001 v0.5 accepted（block_ref/projection/coverage 安定アンカー）
  - DD-XMODAL-001 v0.4 accepted（2軸independence/evaluation_purpose 分離の先行定義）
self_doubt:
  - symmetric cardinality を direction 別に cardinality / cardinality_directional の2 enum に分けたのは過剰設計か、それとも side 順序保持の directional を表現するため必要十分か
  - coverage_complete_for_scope の range 区間演算（union/intersection/⊇）を coordinate_space 別に half-open で定義したが、figure_region（2D bbox）での包含演算が text 系と同列に扱えるか
  - method_registry の emits_candidate_relation + allowed_non_candidate_assertion_types という2軸化が candidate_only の意味矛盾を本当に解消したか
  - decision_payload に assessed_at を含めず digest を安定させたが、同一 payload の再評価が同一 revision_id になる設計でよいか（revision_seq で区別）
questions_for_gpt:
  - must_fix 9点・should_fix 5点が今回で閉鎖されたか
  - 受入試験1, 6, 9, 11 が本文のみで決定的に実装できるか
  - symmetric/directional の cardinality 2 enum 分離が受入試験9を満たすか（未定義分岐ゼロか）
  - coverage_complete_for_scope の range 包含定義が absence/difference gate として十分安全か
  - PASS 圏（PASS_WITH_NOTES 以上）到達可否、残る blocking があれば列挙
decision_requested:
  - PASS可否 / must_fix 9点の個別閉鎖判定 / 受入試験12本の実装可能性判定 / 追加blocking（あれば）
expected_label: DDXDOC_PASS_WITH_NOTES または DDXDOC_MODIFY_REQUIRED
---

# DDXDOC 監査依頼: DD-XDOC-001 v0.6（faceted cross-document comparison & alignment・round5 must_fix 決定式閉鎖版）

- target_mode: inline_embedded（全文を下記に逐語埋め込み）。authoritative bytes = GitHub `asai-dot/Project-codex` ブランチ `claude/daiichi-houki-fact-system-qcn7ph` `docs/dd_candidates/DD-XDOC-001_faceted_cross_document_comparison_candidate_v0.6_20260621.md`（sha256:3774ef64…）。

---

<!-- BEGIN INLINE EMBED: DD-XDOC-001 v0.6 -->
...
<!-- END INLINE EMBED: DD-XDOC-001 v0.6 -->
