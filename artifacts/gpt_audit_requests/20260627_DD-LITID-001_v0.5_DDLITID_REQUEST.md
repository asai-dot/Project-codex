---
request_id: DD-LITID-001-v0.5-20260627
supersedes_request_id: DD-LITID-001-ATTR-20260615 / DD-LITID-FP-20260615 / DD-LITID-PLAN-4route-20260618
topic: DD-LITID-001 v0.5 統合版 — 分散4断片（v0.2-draft / ATTR / FP / PLAN-4route）を単一凍結候補へ畳み、独立性語彙を DD-INDEP-LINEAGE-001 leaf へ一方向 binding（入口と三部作の独立カウント一致）。入口 DD 凍結の可否監査
gate: DDLITID
status: queued
result_expected_filename: 20260627_DD-LITID-001_v0.5_DDLITID_RESULT.md
target_mode: inline_embedded
source_hash: sha256:2f6ed66e1b7f55226713e8dc9ba4b3fb888806c9eb0cdc94b507c56e1939e25f
leaf_pin:
  id: DD-INDEP-LINEAGE-001
  version: v0.1
  content_hash: sha256:a7856be147a3f45b19fafe04d8ad2044565f00d7572a93cd477e144101629b43
  acceptance_ref: RESULT 2306834650821 (DDINDEP_PASS_WITH_NOTES)
absorbed_fragments:
  - DD-LITID-001 v0.2-draft（biblio_identity_noisbn・Box 2275006797196・DESIGN_PASS_WITH_NOTES 2275637858392）
  - DD-LITID-001-ATTR v0.1（属性観測層・Box 2286087523073・DDLITID_ATTR_DESIGN_PASS_WITH_NOTES 2286160319588）
  - DD-LITID-FP（4信号 fingerprint・Box 2287161796030）
  - DD-LITID-PLAN 4route v0.1（運用計画・Box 2292468018025）
review_scope:
  include:
    - 統合の健全性: 4断片を1本に畳んで矛盾・取りこぼし・二重正本が無いか（同一性3層＋holding/authority/source 軸の統一・観測層の対称化）
    - ★独立性 leaf binding（§4）: provenance_group→same_origin_collapse_key / source_lineage→upstream_lineage_id / origin_publisher→content_independence_group 算定入力 / medium_origin・OCR run→observation_lineage_root が正しい写像か。入口が独自に独立カウントを再定義していないか（leaf を pin して一方向 consume か）。同一供給元 TOC 再配信が独立2票にならないか。note5（不明系譜を独立にしない）取り込み
    - 観測層（§2）: append-only 観測=正本／決定的 projection=派生／biblio_item scalar 二重正本でない／時点モデル（取得/主張/版は item 粒度）が健全か
    - anti-hallucination（§6）: 採用値が必ず obs_id 接地・観測なき canonical=0 がデータ層で強制されるか。厚さ≠正しさ・単独権威採用可・Stage1/Stage2 分離
    - fingerprint（§3）: 4信号（pub_date 細粒度/edition/page_count/TOC 主証拠）・弁コム page_count gap の NDL 補完
    - LITLINK を別 DD 維持（§9）の責務分離判断の妥当性（identity vs link/ownership・XDOC OWNERSHIP_NO_REDEF 整合）
  exclude:
    - DDL/DB/既存 lit_id 移行/backfill/mint/OCR/embedding/canonical promotion/serving/Box mutation（HOLD）
    - DD-INDEP-LINEAGE-001 本体の再監査（accepted）
    - 三部作（XMODAL/XDOC/RECONCILE）本体の再監査（ratified）
regression_anchors:
  - DD-INDEP-LINEAGE-001 v0.1 RESULT（Box 2306834650821）DDINDEP_PASS_WITH_NOTES（leaf 正本・note5）
  - 横断整合再監査 v2 docs/research/trilogy_consistency_stability_audit_v2_20260627.md（§2.5 入口独立性ドリフト指摘）
  - DD-LITID-001 v0.2 RESULT（Box 2275637858392）DESIGN_PASS_WITH_NOTES
  - cleaned design record 02_LIT_DESIGN_RECORD_CLEANED_20260615（Box 2286222789037・v0.4 ポインタ）
self_doubt:
  - v0.4 ポインタ（cleaned record）を飛ばして discrete 4断片から v0.5 へ畳んだ版番号付けが監査者に明瞭か
  - PLAN-4route の運用パイプラインを設計 DD に内包したが、運用計画と規範スキーマの分担が混線しないか
  - 観測層を全 scalar/分類へ広げる射程が Stage1 として過大でないか（厚さを作りこむ罠に戻らないか）
questions_for_gpt:
  - 4断片統合に矛盾・取りこぼしが無いか
  - 独立性の leaf binding（§4）が入口と三部作の独立カウント一致を本当に担保するか（反こたつ記事の入口防衛として十分か）
  - 本 v0.5 を DD-LITID-001 の凍結正本として ratify 可能か（凍結後に RECONCILE §7 pin を実値化できる状態か）
  - LITLINK 別 DD 維持の是非
  - 残ドリフト・要修正があれば指摘
decision_requested:
  - PASS可否 / 統合健全性 / 独立性 leaf binding / 観測層・接地 / LITLINK 分離 / 凍結可否 / 追加指摘
expected_label: DDLITID_PASS_WITH_NOTES または DDLITID_MODIFY_REQUIRED
---

# DDLITID 監査依頼: DD-LITID-001 v0.5（統合版・独立性 leaf binding・入口 DD 凍結可否）

- target_mode: inline_embedded。authoritative bytes = GitHub `asai-dot/Project-codex` `docs/dd_candidates/DD-LITID-001_consolidated_candidate_v0.5_20260627.md`（sha256:2f6ed66e1b…）。
- 目的: 実データ投入の**入口 DD**を凍結可能な単一正本に畳む。横断整合再監査 v2 §2.5 が指摘した「入口が独立性を独自語彙で別実装」ドリフトを、**DD-INDEP-LINEAGE-001 v0.1（accepted・leaf 正本）へ一方向 binding** して解消したことの妥当性を問う。
- leaf pin: id=DD-INDEP-LINEAGE-001 / version=v0.1 / content_hash=sha256:a7856be1… / acceptance_ref=RESULT 2306834650821。
- 凍結後の出口: RECONCILE v0.3 §7 の DD-LITID-001 pin（version+content_hash+acceptance_ref）を実値化 → Phase1 dependency gate 緑化。

---

<!-- BEGIN INLINE EMBED: DD-LITID-001 v0.5 consolidated -->
...
<!-- END INLINE EMBED: DD-LITID-001 v0.5 consolidated -->
