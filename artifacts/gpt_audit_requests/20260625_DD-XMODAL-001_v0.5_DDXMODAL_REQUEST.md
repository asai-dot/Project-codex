---
request_id: DD-XMODAL-001-v0.5-20260625
supersedes_request_id: DD-XMODAL-001-v0.2-20260619
topic: XMODAL v0.5 — 独立性 lineage 正本確定（confirmed=content same-origin collapse・observation は scan_source 根）＋law_authority typed contract の独立意味監査
gate: DDXMODAL
status: queued
result_expected_filename: 20260625_DD-XMODAL-001_v0.5_DDXMODAL_RESULT.md
target_mode: inline_embedded
source_hash: sha256:2bddc488d56e6a5195d220b1b87ae280313813014deb89c0baa17c6ed6e89ea2
review_scope:
  include:
    - R1: confirmed の独立を content_origin_root の DISTINCT（same-origin collapse）に確定・observation pipeline の distinct を独立票に数えない・artifact→family binding・family_kind≠origin_object_type
    - law_authority_snapshot typed contract（bitemporal captured/known/valid＋revision_set_hash＋source_hash）への束ね
    - v0.4 不変部（D0/D1/D2・5ガードレール・ストレステスト・inventory-probe）を狭めていないか
    - RECONCILE v0.2 §1 を consume する整合（二重定義でなく付録正本を参照）
  exclude:
    - DDL/DB/mint/OCR/embedding/production（HOLD）
    - v0.4 で accepted の D軸分割・possible_reason 等の再監査
regression_anchors:
  - DD-XMODAL-001 v0.4 accepted
  - DD-TRILOGY-RECONCILE-001 v0.2（§1 lineage 正本・同時起票）
  - DD-TRILOGY-RECONCILE-001 v0.1 RESULT（Box 2306465882050）R1 指摘
self_doubt:
  - external_source_family_registry を語彙として残しつつカウントを lineage に移す設計が混乱を生まないか
  - confirmed を content のみとし observation を独立票から外したが、観測独立（抽出相互確認）は XDOC extraction_corroboration に委ねる分担で十分か
questions_for_gpt:
  - R1 の confirmed 意味確定が lineage 正本として十分堅牢か
  - law_authority_snapshot contract が版差・法改正の落とし込みに十分か
  - RECONCILE v0.2 との二本立てに重複/齟齬がないか
decision_requested:
  - PASS可否 / R1 確定の妥当性 / law contract / RECONCILE との整合
expected_label: DDXMODAL_PASS_WITH_NOTES または DDXMODAL_MODIFY_REQUIRED
---

# DDXMODAL 監査依頼: DD-XMODAL-001 v0.5（独立性 lineage 正本確定・law typed contract）

- target_mode: inline_embedded。authoritative bytes = GitHub `asai-dot/Project-codex` `docs/dd_candidates/DD-XMODAL-001_cross_modal_triangulation_living_corpus_candidate_v0.5_20260625.md`（sha256:2bddc488…）。RECONCILE v0.2（DDRECONCILE）と対で監査。

---

<!-- BEGIN INLINE EMBED: DD-XMODAL-001 v0.5 -->
...
<!-- END INLINE EMBED: DD-XMODAL-001 v0.5 -->
