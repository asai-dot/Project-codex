---
request_id: DD-XDOC-001-v0.8-20260623
supersedes_request_id: DD-XDOC-001-v0.7-20260622
topic: faceted cross-document comparison & alignment candidate v0.8（round7 B14〜B18・coverage完全性/support証拠拘束 閉鎖版）の独立意味監査
gate: DDXDOC
status: queued
result_expected_filename: 20260623_DD-XDOC-001_faceted_cross_doc_v0.8_DDXDOC_RESULT.md
target_mode: inline_embedded
source_hash: sha256:8771e9a88e84528c89ef9fbc58ee6e1127978fd122a61ec533c79d459966dd46
prior_result_file_id: 2302727111997
review_scope:
  include:
    - B14: required_scope_keys(claim_context から導出) ⊆ actual_scope_keys を必須化し未登録 member/scope を false に・claim_is_absence_or_difference を claim_context.claim_kind 型付き入力に接地（受入試験6）
    - B15: support_basis 両配列を unique・cluster_policy.required_basis_types による conditional non-empty・空配列→non_empty_required で effective=false（受入試験11a）
    - B16: support_basis_valid 型別 FK 整合（同一 alignment_observation_id・member ∈ low/high・facet=cluster_facet・coverage は current かつ complete・use_assessment は active かつ allowed purpose/target）（受入試験11b/c/d）
    - B17: coverage_assessment_key_id(logical key に coordinate_space/policy_id 含む)+coverage_revision_seq(integer)+UNIQUE+current=max(revision_seq)+coverage_assessment_id に payload_digest（v9/v10 string 順序非依存）
    - B18: prose gate を executable predicate に（FACET_ANCHORED=unit_ref typed FK・DERIVED=claim_support_eligible literal const false・NO_CLAIM_SUPPORT=record predicate・CLUSTER_SUPPORT_CONTRACT=cluster_status const candidate・method_determinism equality）
    - non-blocking 5点（companion unique/registry存在/facet互換・method_capability_rule_id exactly-one・nondeterministic companion digest・ID immutable derivation contract・independence global invalid active/current 限定）
    - 受入試験12本が本文だけから決定的に実装可能か（特に 1, 6, 11）
  exclude:
    ...
    - v0.7 で CLOSED の B9（companion ID 材料化）/ B10（reviewed none priority85）の再検討
regression_anchors:
  - DD-XDOC-001 v0.7 REQUEST（Box 2302691934758）
  - DD-XDOC-001 v0.7 RESULT（Box 2302727111997）label=DDXDOC_MODIFY_REQUIRED tests 1/6/11 FAIL
  - DD-XDOC-001 v0.6 RESULT（Box 2301891158281）label=DDXDOC_MODIFY_REQUIRED（訂正版）
  - DD-LAYOUT-001 v0.5 accepted / DD-XMODAL-001 v0.4 accepted
self_doubt:
  - required_scope_keys を claim_context.claimed_member_refs × required_coordinate_space で導出したが、claimed_side=both の場合に両 side 全 member を要求する粒度で十分か（過剰/過小）
  - support_basis_valid に cluster_policy.allowed_support_targets/purposes を導入したが、coverage_assessment 側に purpose/target が無いため coverage ref には適用せず use_assessment ref のみに適用した設計でよいか
  - coverage_revision_seq を integer 単調増加にしたが、並行生成（single-writer 前提）でなければ seq 競合し得る点を HOLD/運用前提に委ねてよいか
  - cluster_status=candidate を定数にしたが、将来 promotion を別 DD に出す際 enum 拡張で後方互換を壊さないか
questions_for_gpt:
  - B14〜B18 の全 blocking が今回で閉鎖されたか
  - 受入試験 1, 6, 11 が本文のみで決定的に実装できるか
  - support_edge_effective が空配列（non_empty_required）・FK 非整合（support_basis_valid）・failed current coverage を全て false にできるか
  - coverage_complete_for_use_assessment の required ⊄ actual → false が missing member/scope を確実に検出するか
  - PASS 圏（PASS_WITH_NOTES 以上）到達可否、残る blocking があれば列挙
decision_requested:
  - PASS可否 / B14〜B18 の個別閉鎖判定 / 受入試験12本の実装可能性判定 / 追加blocking（あれば）
expected_label: DDXDOC_PASS_WITH_NOTES または DDXDOC_MODIFY_REQUIRED
---

# DDXDOC 監査依頼: DD-XDOC-001 v0.8（faceted cross-document comparison & alignment・round7 coverage完全性/support証拠拘束 閉鎖版）

- target_mode: inline_embedded（全文を下記に逐語埋め込み）。authoritative bytes = GitHub `asai-dot/Project-codex` ブランチ `claude/daiichi-houki-fact-system-qcn7ph` `docs/dd_candidates/DD-XDOC-001_faceted_cross_document_comparison_candidate_v0.8_20260623.md`（sha256:8771e9a8…）。

---

<!-- BEGIN INLINE EMBED: DD-XDOC-001 v0.8 -->
...
<!-- END INLINE EMBED: DD-XDOC-001 v0.8 -->
