...
gate: DDLAYOUT
status: queued
result_expected_filename: 20260619_DD-LAYOUT-001_page_geometry_v0.1_DDLAYOUT_RESULT.md
target_mode: inline_embedded
source_hash: sha256:dcd893ff33627bf356489c3b22212ee24fc26af25bb62b5c612295d8d386c893
review_scope:
  include:
    - 幾何面 page_block（bbox/block_type/reading_order/char_span/toc_node の橋渡し）の設計妥当性
    - toc_locator（目次タップ→該当ページ）の内包設計
    - 両生成経路（bbox_ocr 主 / retrofit_from_existing_ocr 従）の first-class 併存
    - 更新・付け加え・修正パターンの append-only / current-view 設計の十分性
    - 自己修復（asset_content_hash 不一致時に幾何のみ再整列・1:1 非前提）の健全性
    - 本文(LIT)と書式(FORMOBJ)の共有基盤化（FORMOBJ の bbox 欠落補完）
    - 既存スパイン（source_text_revision / toc_nodes / LITCHUNK / LITLINK）への接地の正しさ
  exclude:
    - 三軸インデックス（源泉×分野×法域）の中身の是非（v0.3 で既決）
    - 要件事実の claim_element_unit 降格（v0.3 で既決）
    - DD-LITRIGHTS-001 不作成方針（既決・蒸し返さない）
    - biblio.toc_nodes 本番投入の事実（552,544 行・DDTOCNODES_PASS 既決）
    - FORMOBJ 4層分離・form_uid opaque（owner ratify 済）
regression_anchors:
  - DD-FORMOBJ-002 v0.2（Box 2286527242803）form_witness locating・gate_witness_edition_verified
  - 01_TOC DD v0.1（Box 2279844275881）raw/observation/alignment/candidate/current・claim_support_eligible=false
  - DDTOCNODES production apply（Box 2285740892425）biblio.toc_nodes 552,544 / 列 book,title,page,depth,path,parent,source,status
  - 02_LIT_DESIGN_RECORD_CLEANED（Box 2286222789037）文献 object spine（asset/text/chunk/link）
  - 三軸インデックス設計 v0.3（Box 2287660763513）research_unit・claim_support 二層
  - bookshelf_supabase_schema_draft（Box 2205460643463）正本H側 / Box release store / Supabase read-mostly catalog / active_release pointer
  - DD-BOOKQ-001 v0.2（Box 2257663257570）books.json digital.pdf_quality
  - LIT_SOURCE_GAP_20260618（Box 2292462074338）investment HOLD・mint 待ち
self_doubt:
  - 幾何面を独立 first-class にすべきか、LITASSET/TOC の下位産物に留めるべきかの線引きが弱い
  - retrofit(B) の char_span 整列の現実精度（既存OCRに座標が無い前提）に不確実性
  - reading_order の挿入耐性キー（実数 / LexoRank 等）が未確定（O4）
  - catalog 投影先（現 catalog.toc_index は page を持たない）の置き方が未定（O5）
  - bbox 正規化座標系の規約（原点・y軸向き・回転表現）未確定（O2）
  - H 側ローカルに DD-LAYOUT 相当が既存でないかを Box からは確認しきれない（O1・再発見リスク）
questions_for_gpt:
  - 本 candidate を「設計 closure 候補」として前進させてよいか
  - 幾何面を 7 オブジェクト横断の独立 first-class レイヤに切り出す方針の是非（FORMOBJ「親なし関係エッジのみ」原則との整合）
  - net-new を page_block ＋ toc_locator の 1 レイヤに限る最小性は過小/過大でないか
  - ページ番号を同一性にせず char-offset＋content_hash を真とする設計は再OCR・軽量版再生成に本当に壊れないか
  - ratify 前ゲート（O1 H側突合・O7 canonical配置=CANONICAL_MAP従属）の十分性
decision_requested:
  - PASS可否 / 設計closure候補としての前進可否
  - 幾何面 first-class 化の採否
  - 最小性（page_block+toc_locator）の妥当性判定
  - ratify前に追加で解くべき must_fix の指摘
expected_label: DDLAYOUT_PASS_WITH_NOTES または DDLAYOUT_MODIFY_REQUIRED（approach reject ではなく must_fix/should_fix 反映で前進できる形を希望）
---

# DDLAYOUT 監査依頼: DD-LAYOUT-001 v0.1（page 幾何・text-region 面）

## 監査対象について
- 本 REQUEST は **target_mode: inline_embedded**。監査対象 DD の全文を下記「===== 監査対象本文 =====」に**逐語**で埋め込む。
- 同一本文の正本候補は GitHub `asai-dot/Project-codex` ブランチ `claude/daiichi-houki-fact-system-qcn7ph` の
  `docs/dd_candidates/DD-LAYOUT-001_page_geometry_text_region_plane_candidate_v0.1_20260619.md`（Draft PR #32）。
- source_hash（sha256）= `dcd893ff33627bf356489c3b22212ee24fc26af25bb62b5c612295d8d386c893`（上記リポファイルの値。埋め込み本文はその逐語コピー）。
...
## 特に見てほしい論点
1. first-class 切り出しの是非（FORMOBJ「親を持たせない」原則との整合）。
2. 同一性＝char-offset＋content_hash が再OCR・軽量版再生成に対して壊れないか。
3. 両生成経路（bbox_ocr / retrofit）の provenance 併存（上書きしない）設計の健全性。
4. 更新/追加/修正の append-only＋current-view＋lineage の十分性・抜け。
5. claim_support 不可ゲートが TOC由来参照禁止ゲートと一貫しているか。
6. 最小性（net-new = page_block＋toc_locator）の妥当性。

## 既知の弱点・open（本DD §9）
O1 H側 DD-LAYOUT 突合（ratify前ゲート） / O2 座標系規約 / O3 block_type 語彙 / O4 reading_order 挿入耐性キー /
O5 catalog 投影スキーマ / O6 retrofit 抽出器・confidence 規約 / O7 canonical 配置（CANONICAL_MAP 従属）。
