---
request_id: DD-XDOC-001-v0.9-20260623
supersedes_request_id: DD-XDOC-001-v0.8-20260623
topic: faceted cross-document comparison & alignment candidate v0.9（round8 B19〜B23・claim完全性/support scope拘束 閉鎖版）の独立意味監査
gate: DDXDOC
status: queued
result_expected_filename: 20260623_DD-XDOC-001_faceted_cross_doc_v0.9_DDXDOC_RESULT.md
target_mode: inline_embedded
source_hash: sha256:91f73f6ca0c081fefab499b130a05a8ab402df3d9cb8af802c93817d5e5c96b4
prior_result_file_id: 2303196102767
review_scope:
  include:
    - B19: claim_member_selector 型付き（all_on_side=expected_member_set 完全一致 / explicit_subset=subset_selector_id hash 固定）で claimed_member_refs 過少申告を封じたか（受入試験6a）
    - B20: coverage_scope_binding_valid（scope.member_ref=ca.member_ref・ca.alignment=use_assessment.alignment・ca.side=member side・facet/policy 一致・current 1件）で無関係 coverage 差替えを封じたか（受入試験6b）
    - B21: support_basis_coverage_scope_refs[]（scope+assessment 対参照）+ covers_all_required_members（low/high 双方 complete）で required_scope_for 未接地を解消したか（受入試験11c/d）
    - B22: required_basis_types/allowed_support_eligibilities minItems=1・hold は allowed_support_eligibilities 外（既定 eligible）で evidence-free policy/hold support を封じたか（受入試験11a/b）
    - B23: support_edge_key_id/revision_seq/supersedes/support_status で basis 変更を append-only 化したか（受入試験11e）
    - nb1 coverage payload canonical sort/merge 後 digest（入力順非依存）・nb4 specificity 決定式・nb5 reproducibility 保全
    - 受入試験6・11 が本文だけから決定的に実装可能になったか
  exclude:
    - DDL/DB/mint/Box mutation/OCR/embedding/production実装（HOLD）
    - DD-LAYOUT-001 v0.5 / DD-XMODAL-001 v0.4 の内容変更（依存先・既accepted）
    - v0.8 で CLOSED/PASS の B9/B10/B17/B18・§7 eligibility priority・§8 method capability・cardinality 周辺の再検討
regression_anchors:
  - DD-XDOC-001 v0.8 REQUEST（Box 2302883061470）
  - DD-XDOC-001 v0.8 RESULT（Box 2303196102767）label=MODIFY_REQUIRED tests 6/11 FAIL
  - DD-XDOC-001 v0.7 RESULT（Box 2302727111997）
  - DD-LAYOUT-001 v0.5 accepted / DD-XMODAL-001 v0.4 accepted
self_doubt:
  - explicit_subset を許す設計（subset_selector_id を hash 固定）で「自由省略を記録として残す」方針が、過少申告防止として all_on_side 強制より緩くないか
  - support_basis_coverage_scope_refs に member_ref を冗長保持したが、scope 側 member_ref との二重管理が不整合を生まないか
  - support_edge_revision_id に basis_digest を含めたことで、同一 basis 集合の再評価が同一 revision になる設計でよいか（revision_seq で区別）
  - allowed_support_eligibilities 既定 eligible のみ・hold は tentative_support 分離としたが、tentative graph の用途を別 DD に委ねてよいか
questions_for_gpt:
  - B19〜B23 の全 blocking が今回で閉鎖されたか
  - 受入試験6・11 が本文のみで決定的に実装できるか
  - coverage_scope_binding_valid + support_basis_valid_scope で「無関係 member の完全 coverage 差替え」が全経路で封じられたか
  - PASS 圏（PASS_WITH_NOTES 以上）到達可否、残る blocking があれば列挙
decision_requested:
  - PASS可否 / B19〜B23 の個別閉鎖判定 / 受入試験6・11 の実装可能性判定 / 追加blocking（あれば）
expected_label: DDXDOC_PASS_WITH_NOTES または DDXDOC_MODIFY_REQUIRED
---

# DDXDOC 監査依頼: DD-XDOC-001 v0.9（faceted cross-document comparison & alignment・round8 claim完全性/support scope拘束 閉鎖版）

- target_mode: inline_embedded（全文を下記に逐語埋め込み）。authoritative bytes = GitHub `asai-dot/Project-codex` ブランチ `claude/daiichi-houki-fact-system-qcn7ph` `docs/dd_candidates/DD-XDOC-001_faceted_cross_document_comparison_candidate_v0.9_20260623.md`（sha256:91f73f6c…）。

---

<!-- BEGIN INLINE EMBED: DD-XDOC-001 v0.9 -->
...
<!-- END INLINE EMBED: DD-XDOC-001 v0.9 -->
