---
request_id: DD-XDOC-001-v0.7-20260622
supersedes_request_id: DD-XDOC-001-v0.6-20260621
topic: faceted cross-document comparison & alignment candidate v0.7（round6 B9〜B13・接続字段/predicate 閉鎖版）の独立意味監査
gate: DDXDOC
status: queued
result_expected_filename: 20260622_DD-XDOC-001_faceted_cross_doc_v0.7_DDXDOC_RESULT.md
target_mode: inline_embedded
source_hash: sha256:1a025c85e28ff22c83bc202cd77a9eb377fd685bb691bfcfc330cfe2f8509ba9
prior_result_file_id: 2301891158281
review_scope:
  include:
    - B9: xdoc_alignment.applied_companion_method_registry_ids[] を required 字段化し ID 材料に含めたか。G_XDOC_METHOD_CAPABILITY_DECLARED が required companion ⊆ applied を検査（受入試験7）
    - B10: priority=85 で reviewed_relation_type=none を positive target で ineligible・purpose_positive_reviewed_relations 集合で NOT NULL を列挙置換（受入試験12）
    - B11: coverage_claim_scope.required_ranges minItems=1・coverage_complete_for_use_assessment 全scope AND 集約・figure_region を rect_2d adapter に分離・covered_ranges=[] で必ず false（受入試験6）
    - B12: xdoc_support_edge に support_basis_use_assessment_revision_ids[]/support_basis_coverage_assessment_ids[] を持たせ実在字段で support_edge_effective 再記述・coverage_status revision 規則（受入試験11）
    - B13: 単一 cardinality enum（symmetric→n_one 禁止 constraint）・members_a/b minItems=1・method_determinism 字段・gate→predicate 接地表§11（受入試験1）
    - should_fix 4点（decision digest に revoked・coverage_assessment_id に input fingerprint・member-level outlier・score_components_ref immutable）
    - 受入試験12本が本文だけから決定的に実装可能か（特に 1, 6, 7, 11, 12）
  exclude:
...
regression_anchors:
  - DD-XDOC-001 v0.6 REQUEST（Box 2300656016821）
  - DD-XDOC-001 v0.6 RESULT（Box 2301891158281）label=DDXDOC_MODIFY_REQUIRED（訂正版・prior PASS_WITH_NOTES を格下げ）
...
self_doubt:
  - cardinality を単一 enum に統合し symmetric→n_one 禁止 constraint を置いたが、conditional 2-enum 廃止で受入試験1/9 を満たすか（GPT 推奨は単一 enum 又は明示分離の二択）
  - applied_companion_method_registry_ids[] を ID 材料に含めると、同一 member/primary で companion 違いの alignment が別 observation になる設計でよいか
  - coverage の covered∩unknown 対象内重複を validation error にしたが、unknown 優先で安全側に倒す選択肢の方が望ましいか
  - support_basis_*_ids[] を edge に直接持たせたが、method_run_bundle 新設案（B9案2）と比べ最小か
questions_for_gpt:
  - B9〜B13 の全 blocking が今回で閉鎖されたか
  - 受入試験 1, 6, 7, 11, 12 が本文のみで決定的に実装できるか
  - reviewed none の priority=85 + positive 集合列挙の二重防御で意味矛盾が消えたか
  - support_edge_effective が実在字段（support_basis_*_ids + assessment_status/coverage_status 導出）のみで判定可能になったか
  - PASS 圏（PASS_WITH_NOTES 以上）到達可否、残る blocking があれば列挙
decision_requested:
  - PASS可否 / B9〜B13 の個別閉鎖判定 / 受入試験12本の実装可能性判定 / 追加blocking（あれば）
expected_label: DDXDOC_PASS_WITH_NOTES または DDXDOC_MODIFY_REQUIRED
---

# DDXDOC 監査依頼: DD-XDOC-001 v0.7（faceted cross-document comparison & alignment・round6 接続字段/predicate 閉鎖版）

- target_mode: inline_embedded（全文を下記に逐語埋め込み）。authoritative bytes = GitHub `asai-dot/Project-codex` ブランチ `claude/daiichi-houki-fact-system-qcn7ph` `docs/dd_candidates/DD-XDOC-001_faceted_cross_document_comparison_candidate_v0.7_20260622.md`（sha256:1a025c85…）。

---

<!-- BEGIN INLINE EMBED: DD-XDOC-001 v0.7 -->
...
<!-- END INLINE EMBED: DD-XDOC-001 v0.7 -->
