# LEASE_SUBSYSTEM_DESIGN v0.5 — mutating レーン解禁の最小設計（design 確定版）

- status: **RATIFIED (owner-approved 2026-06-25 / asai) — design 確定・未実装**
  - 監査経緯: v0.1→v0.5 の 5 ラウンド GPT Pro design audit。v0.4 result 2306561290821 で
    新規 blocking は 8→2 件まで収束（B9/B12/B14/B15 CLOSED, B10/B13/B16 PARTIAL=運用前注記）。
  - **本 v0.5 をもって design gate を closeする**（オーナー判断）。GPT の次ラウンドは回さない
    （監査者基準が「曖昧ゼロ」で構造的に fixed point を持たないため、設計確定はオーナー裁定で行う）。
  - 残 2 件（B11 payload binding / B17 lease_echo）を本版で閉じ、B18 は **次 gate（実 mutation）
    の契約**として記述のみ（本版では未実装）。
- date: 2026-06-25
...
## 9. owner 確定事項（v0.5 ratify）

- design gate は v0.5 で close。GPT 再監査は回さない。
- 実装着手は別タスク（offline validator prototype + fixtures から）。feature flag 既定 off 継続。
- HANDOFF schema canonical patch / authorization registry version 管理 / HMAC 鍵運用 / recovery
  queue runbook は **operational gate 前**に別途確定（本 design の前提として明示繰延）。
- 実 mutation 解禁（§8）は将来の独立 gate。実務で mutating が必要になるまで着手しない。
