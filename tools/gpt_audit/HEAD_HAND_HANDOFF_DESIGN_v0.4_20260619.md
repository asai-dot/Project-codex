# HEAD_HAND_HANDOFF_DESIGN v0.4 — ヘッド↔ハンドのシームレス受け渡し設計
...
- owner 決定:
  ...
  - **effect は3軸**: `mutation_class`（lease）/ `egress_decision`（機密送信）/
    `resource_effect_class`（課金・quota・rate-limit・外部ログ）。read は mutation 軸で
    non_mutating。無償 public read は広く通す。**有料/quota/rate-limited read は resource
    permit 必須＝permit 不在なら fail-closed で blocked**。
...
正は付録 §3（`handoff-dispatch/0.4`）。要点:
- integrity（付録 §6）: `packet_hash`（local が render 後・公開前に計算・**attempt_id 非包含**）、
  `hash_basis: rfc8785_jcs`（単一方式・JCS 接地）、`excluded_from_hash` 完全列挙、
  `source_artifacts`（`version_ref_type` 型付き）。
- routing: `governance_role`（gate metadata）/ `execution_role` / `assignee`
  （invalid 明示値は validation error）。
- **3軸 effect（付録 §4）**: `mutation_class` / `egress_decision` / `resource_effect_class`
  ＋ `side_effect_flags`（状態変更のみ）/ `egress_descriptor` / `resource_descriptor` /
  `lease_required` / `resource_permit_required`。
...
reconciliation event schema（付録 §5.2）に保存。evidence grade は local 算定。semantic
同値は付録 §5.4 の規則。
...
- **operational implementation**（HOLD）: v0.4 閉鎖＋owner ratify 後、non_mutating レーン
  から。mutating/lease/resource permit は別 gate まで HOLD。
- スキーマは付録を単一の正とする。
