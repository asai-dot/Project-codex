...
## 第4ソース投入（2026-06-02・追補）
Box `decrypted/法律用語辞書.txt`＝「**法律用語がわかる辞書 第2版**」(CASIO EX-word抽出)の
見出し語＋読み **2,694対**（`data/wakaru/wakaru_jiten_reading.jsonl`。定義本文は暗号化で不可）。
連濁 review 48件に第4投票として適用 → カバー7件:
- 新規confirmed 4件（JLT＋わかる多数決で有斐閣誤り確定）→ 台帳に追記。確定計 37→41。
  例: 親等 いっしんとう→しんとう、財産分離 さいさん→ざいさん、転質 てんじち→てんしち、暫定予算 さんてい→ざんてい。
- variant確定 3件（JLT以外2源一致＝連濁の正当異読、誤りでない）: 保健所 ほけんじょ／商事会社 がいしゃ／選挙権。
  ※保健所は JLT「ほけんしょ」が少数派＝JLTも万能でない実例。
- 残り41件は わかる辞書 に無く未決（さらに高被覆ソース＝岡口辞書等が要る）。
