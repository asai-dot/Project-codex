...
-- 再学習除外: inference_log に outcome enum(accepted/rejected/superseded) を持ち、
--   rejected/superseded は教師データから除外する（モデル再学習時のフィルタ条件）
```
...
CREATE TABLE dynamic_alo.party_alias (
    ...
    alias_value_norm  text,                          -- 正規化（※候補生成用、確定キーにしない: GPT§中6）
