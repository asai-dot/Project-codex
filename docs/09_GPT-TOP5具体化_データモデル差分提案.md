...
> ⚠️ **訂正（2026-06-07）**: 「Boxに新規メタデータ付与」は不要。各案件フォルダに **`_alo_dir.json`(alo-dir-map-v2)** が `sf`・`alo_matter_uri` を保持し、集約 `alo_dir_index_accum.json` もある。DB側は `external_link` に取り込むだけ。やるべきは**新規フォルダで `_alo_dir.json` を確実に生成・最新化**。詳細 [`10`](10_AIフィット向け前提資料_引き渡しチェックリスト.md)。以下は当初提案（参考）:
