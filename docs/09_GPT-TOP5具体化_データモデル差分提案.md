...
### ② Box `sf_record_id` は `_alo_dir.json` で既に存在（訂正）＋`external_link`
> ⚠️ **訂正（2026-06-07）**: 「Boxに新規メタデータ付与」は不要。各案件フォルダに **`_alo_dir.json`(alo-dir-map-v2)** が `sf`・`alo_matter_uri` を保持し、最新最広の集約は DD-BOXCATALOG-001 常設索引（495案件/59,017ファイル/SF照合403。古い`alo_sf_box_index_v1`は不使用）。DB側は `external_link` に取り込むだけ。やるべきは**新規フォルダで `_alo_dir.json` を確実に生成・最新化**。詳細 [`10`](10_AIフィット向け前提資料_引き渡しチェックリスト.md)。以下は当初提案（参考）:
