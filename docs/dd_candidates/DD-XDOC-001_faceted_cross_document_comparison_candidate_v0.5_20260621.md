...
**注**：frbr_work_candidate × frbr_work の eligible（priority=50）は单一 alignment の use_assessment が 1片の証拠を示すものに過ぎない。FRBR Work 昇格には外部で multi-facet cluster 証拠の確立が必要（cluster-level 契約・HOLD）。
...
上記組合せ以外に listed されていない origin_relation × candidate_relation_type からの reviewed_relation_type 設定は validation error。
