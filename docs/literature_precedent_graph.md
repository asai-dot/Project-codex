...
|---|---|---|---|
| ① | 所内の生判決文（PD・著作権なし） | 内部ビューワー | ◎ ALO所有。**事務所自身の係属事件**（leala `JurisdictionCourt`: 事件番号・裁判所が構造化済）が主要供給源 |
...
3. **条文ジャンプ（旧条→新条）— ⏸ 保留タスク（watch、§11）**: 新旧対照表/附則から `旧167条→現166条` の
   対応を構築。**重く、かつ外部で整備されそう**なので自前構築は急がない（§11参照）。
...
自前で作らず「フラグだけ立てて様子見」する項目。効率的な外部データ源/手法が出たら採用に切替。

| # | 項目 | なぜ保留 | 採用に切り替える条件（trigger） |
|---|------|---------|-------------------------------|
| W1 | **条文レベル 旧→新 対応データ**（旧167条→現166条の genealogy） | 構築が重い。デジタル庁の法令標準XML/改正自動生成、第一法規・FRAIMの実証等、**外部で整備されそう**。法令レベル(Phase1)＋時点警告で当面のペインは解消できる。 | ①新旧対照表/附則の機械可読データ（XML等）が公開される ②e-Gov等が条文 genealogy API を出す ③大改正1本でも費用対効果が見合う見込みが立つ → そのとき大改正優先で着手 |

> 方針: W1 が解決するまでは「②当時版の条文へ着地（e-Gov v2時点指定）＋①時点警告バッジ」で運用。
> 条文ジャンプ(③)は無くても、法律家の沈黙の誤着地は①②でほぼ防げる。
