# legallib → biblio 取込プラン v0.5.2（Phase 0 実測反映 ＋ INGEST再監査用）

> status: レビュー用（v0.5.2）。**v0.1〜v0.3（alo.works/toc_nodes/fingerprints/メダリオン）は撤回・superseded。**
>
> **v0.5.2 の差分**: Mac ワーカーの Phase 0 / 0.1 実測（2026-06-08）を反映 → §H。
> 生JSONの実キー確定（`children`/`authors_raw`/`pub_year_raw`）、**詳細TOC取りこぼしバグを再帰展開で修正**
> （dry-run bib_toc **38,029 → 661,141、17.4倍回復**、levels[1..7]）、content_type 4種（book/journal/pubcom/material）
> 判明、未決5点を事実確定。INGEST_NEED_MORE の PASS6点を充足。初回投入スコープ＝**book-only(2,760)**。
...
# v0.5.2 増分（Phase 0 / 0.1 実測反映）

Mac ワーカーが Phase 0 / 0.1 を実行（2026-06-08, RPT:
`from_worker/RPT_legallibbiblio_phase0_1_20260608_1412.md`）。実測で生JSONを確定し、重大バグを1件修正。

## §H-1 生JSON 実キー確定（mapping table）
| biblio 列 | legallib 実キー | 備考 |
|---|---|---|
| title | `title` | |
| responsibility/著者 | **`authors_raw`** | 文字列。複合著者は非分割（bencom方針） |
| publisher | `publisher` | book/journal は概ね有・pubcom/material は無が多い（§H-3） |
| pub_year | **`pub_year_raw`**（先頭4桁を抽出） | |
| isbn | `isbn` | あれば |
| content_type | `content_type` | book/journal/pubcom/material の4種 |
| bib_toc text/level/page | TOCノード `text`/`children`ネスト深さ/`page` | **子配列キー=`children`** |

## §H-2 【重大】詳細TOC取りこぼしバグ修正（最大の差分）
- 旧 `extract_toc_nodes` は **トップ階層(level=1)のみ**採取 → dry-run bib_toc 38,029（≒9.4/冊）と過少だった。
- 生TOCは **`children` 階層**を持つ。**再帰展開（pre-order DFS）に修正**（commit e0003de）。
- 結果: dry-run bib_toc **38,029 → 661,141（17.4倍回復）**、`levels=[1..7]` の多段を確認。
  ＝**詳細TOC（本データの価値の核）を完全取り込み**。ordinal は行に対する0始まり連番（locator, v0.5 §B 不変）。

## §H-3 content_type 4種の landscape（Phase 0.1 実測）
| content_type | 件数 | form_type マップ | publisher NULL | author NULL |
|---|---|---|---|---|
| book | 2,760※ | BOOK | 8 | 38 |
| journal | 422 | PERIODICAL | 0 | 422（誌全体は単一著者を持たない＝仕様）|
| pubcom | 655 | PUBCOM | 655 | 655 |
| material | 214 | MATERIAL | 0 | 214 |
| **計** | **4,051** | | | |
※ book 2,760 は WO 想定 2,751 +9。+9 は `templates_source_input.jsonl` 由来（書式元書籍）、うち3件は pubcom/material にも混入。

## §H-4 未決5点の確定
1. **+9件差**: 許容。書式元書籍由来。biblio は重複を許し authority で統合（sanity ⑨/⑩ で観測）。
2. **publisher/author NULL**: 許容。biblio は忠実保全で列 NULL 可（pubcom は両者ほぼ全NULL＝性質は authority で確定）。
3. **雑誌 TOC page NULL**（4,713/12,657=37.2%）: 許容。bib_toc.page は NULL 可。
4. **詳細TOC**: **全階層投入で確定**（§H-2 で回復）。
5. **巻末索引**: トップレベル index キーは0件。`toc` 内に「索引」系ノード 1,296件が内在。
   → **初回は bib_toc に忠実保持**（実際に目次構造内にある）。`label LIKE '%索引%'` での機械除外は
   誤検出リスク（例『判例索引の使い方』）があり**採らない**。terms(`LEGALLIB_RAW_INDEX`)への正式分離は
   構造ベースで後フェーズ（authority/serving）。

## §H-5 初回投入スコープの決定
- **Phase 1a = book-only（2,760冊）**。最も性質が確定しており PASS 最短。`--book-only` で実行。
- **journal / pubcom / material（計1,291）= Phase 1b（次スコープ）**。実態は把握済（§H-3）だが、
  - journal: 記事 entity 化（title+著者分解）は別タスク（記事パーサ）。biblio へは PERIODICAL として後続投入可。
  - pubcom/material: コンテンツ性質（コンメンタール？書式？）の characterization を経てから form_type を確定。
- loader は4種すべて form_type マップ済（`CONTENT_TYPE_TO_FORM`）。スコープは実行フラグで制御。

## §H-6 INGEST_NEED_MORE → PASS 6点の充足状況
| # | 要求 | 充足 |
|---|---|---|
| 1 | 生JSON 3サンプル | ✅ Phase 0.1 RPT に3サンプル＋深いraw toc 30行インライン |
| 2 | 確定 mapping table | ✅ §H-1 |
| 3 | `--dry-run --limit 3` 出力 | ✅ RPT C |
| 4 | 既存 asai-bookshelf/bencom 差分0 test | ⏳ Phase 1 直前に sanity ⑪ 前後突合（dry-run は既存不読のため設計上0） |
| 5 | 再実行 diff 0 | ⏳ Phase 1 で全件二回流して確認（冪等は決定論ID＋upsertで保証） |
| 6 | 著者誤統合0の初期ID戦略 | ✅ per-occurrence 既定（sanity ⑩ 構造保証） |

→ 1/2/3/6 は完了。4/5 は「実投入を伴う test」のため Phase 1 実行時に証跡化（Phase 0 では原理上不可）。
この点を明記して PASS_WITH_NOTES を要請する。
