...
- ⚠️ **biblio.authors(2,200) と authority.person(128,081) が別系統・未連結**（書籍著者 vs 論文著者）。統合 or リンクが必要（dup温床）。
  - **ヒット率実測(2026-06-27, read-only)**: 自動連結可能（空白除去で一意一致）= **362/2,200 = 16.5%**（曖昧48・不一致1,790）。
  - **データ品質の壁**: 2,200件中 **962件(44%)が「、/,」区切りの複数著者連結文字列** → 単一突合不可。**連結著者の分解が前処理として必須**。`name_yomi` は全NULLで使えず。
  - 単一氏名1,238件に限れば一意一致29.2%。→ 自動連結は3割未満、残りは resolution_log 名寄せ前提。
