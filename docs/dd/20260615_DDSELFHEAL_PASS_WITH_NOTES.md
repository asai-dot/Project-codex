...
- **決定的 repairer 4種** (GPT auto 候補に接地・全て raw 不変の派生 plan):
  - `repair_offset.py` `OffsetPageConvert` (DET_NO_CANONICAL): 検証済み本単位 offset
    (conf1.0/validated/anchors≥2) で pdf_page→print_page を派生。
  - `repair_sha.py` `BodyShaRecompute` (DET_NO_CANONICAL): 解決済み source_content から
    欠落 source_sha256 を再計算 (内容不変)。
  - `repair_normalize.py` `NormalizeTitleRegen` (DET_NO_CANONICAL): 生 title から
    title_norm を再生成 (生 title 不変)。
  - `repair_quarantine.py` `QuarantineOrphan` (QUARANTINE_ONLY): **2ソース以上**で多源照合に
    失敗した orphan のみ reason_code 付き隔離 (単一ソースの自明 orphan は過剰隔離しない / silent drop なし)。
...
- `tests/test_repair.py` 46 checks: 物理書込ゼロ / C0 は write_allowed=0 / whitelist 無しは
  物理拒否 / rollback 可逆 / plan 決定的・冪等 / decision_log chain ok / 単一ソース過剰隔離なし。
