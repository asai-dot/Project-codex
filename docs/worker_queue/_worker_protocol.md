...
11. **PII 厳禁（秘匿特権）**: 一般成果物に個人名・法人名・案件名・住所・電話・メール・事件内容・
    金額・本文を**転載しない**。実データに触れても成果物では必ず**マスク済みプレースホルダ**
    （`<当事者名>` / `個別相談サンプルA` / `<案件フォルダ masked>` 等）に抽象化する。
    結合検証・件数例・サンプル提示でも**生の固有名を書かない**。集計値・構造・ID・命名様式のみ可。
    司令塔は固有名を含む成果物を**監査で差し戻す**（電話/メール正規表現だけでなく固有名スキャンを行う）。
