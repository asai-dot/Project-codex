---
worker_task_id: W-20260624-102
status: queued
priority: P0
owner: claude-code-worker
created_at: 2026-06-24
request: docs/workflow_model/REQUEST_v0.2.md
poc: PoC1
goal: 245問を『実データ/既存資料/実物サンプル/担当者聴取/設計判断』の5区分に分類し、P0 58問を『回答済・証跡あり/暫定回答・追加検証要/浅井聴取必須/中森聴取必須/外部SE確認必須』に振る。02_ヒアリング質問シートの回答状態列を更新
mode: implementation
requires_systems:
  - repo(v0.1)
  - Box(既存設計資料)
depends_on:
  - W-20260624-101
allowed_paths:
  - docs/workflow_model/v0.2/
allowed_actions:
  - read_files(v0.1)
  - classify
  - write deliverable
forbidden_actions:
  - production_db_write
  - salesforce_write
  - box_write_move_rename_delete
  - file_move_rename_delete
  - ai_estimate_as_human_decision
  - pii_in_general_artifact
  - raw_source_mutation
  - human_gate_bypass
  - fill_unknown_with_generalities
  - stall_whole_run_waiting_for_input
exit_criteria:
  - 245問すべてに区分が付き、空欄が無い（§3 空欄禁止）
  - P0 58問すべてに4ステータスのいずれかが付いている
  - 『調査で埋まる質問』を浅井質問に回していない（§7）
  - RESULT を done/ または blocked/ に書く
  - 成果物は docs/workflow_model/v0.2/ 配下にのみ書き出す
  - 未確認事項は推測で埋めず queue に積み、確認状態を明示する
deliverables:
  - docs/workflow_model/v0.2/PHASE1_question_triage_v0.2.csv
max_attempts: 2
result_expected_filename: W-20260624-102_RESULT.md
---

# Task — Phase1 245問を5区分に分類＋P0 58問にステータス付与

発注元: `docs/workflow_model/REQUEST_v0.2.md`（v0.2 指示原本）。前提方針 = Box正本 / SF制御塔 / 自然発生データ優先 / Raw・Canonical・Derived分離。

## Goal
245問を『実データ/既存資料/実物サンプル/担当者聴取/設計判断』の5区分に分類し、P0 58問を『回答済・証跡あり/暫定回答・追加検証要/浅井聴取必須/中森聴取必須/外部SE確認必須』に振る。02_ヒアリング質問シートの回答状態列を更新

## Do Not（§2 絶対原則 / §7 禁止事項）
- 本番(SF/Box)を書き換えない・ファイルを移動/改名/削除しない。
- 実物を見ずに一般論で埋めない。不明は `未確認`/`証拠不足`/`浅井聴取要` を明示。
- AI推定を人間の確定判断として記録しない。観測/推定/人間判断を分ける。
- 一般成果物に個人名・住所・電話・事件内容・具体的金額を書かない。
- 「追加情報待ち」で作業全体を止めない。確認不能はキューに積み、他を進める。

# 注
浅井先生へ聞く前に、既存資料・実データで埋められるものは全部埋める（§3 Phase1）。
