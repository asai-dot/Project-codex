...
**E-1. 導出（取得ゼロ）**: `1000`系3,654 の NRID→eradCode 導出規則を**1件で確証** → 全件導出。person_identifier に投入（候補, source='nrid_derived'）。
...
- `1000`系 NRID→eradCode 導出規則（8桁の取り出し位置）。既知ペア1件 or CiNii NRID仕様で確定。
- KAKEN 研究者API が氏名/研究者番号で逆引き可能か、レート（concurrency 4上限は既知）。
- 9000系 scholar のうち実際に科研費を持つ割合（E-2サンプルで判明）。
