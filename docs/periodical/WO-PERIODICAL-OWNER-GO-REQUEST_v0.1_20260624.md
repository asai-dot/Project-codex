...
## 承認記録（owner）
- **2026-06-24 owner(asai@asai-lo.com): 全GO(0–5)承認**。理由「非破壊」。
- head注記: GO-0/1/2 は read-only/dry-run のため即着手。
- **GO-3/4/5(HIGH-HOLD)**: 破壊性は無い(quarantine→破棄ロールバック)が、atlas governance は destructiveness と別に
  `PROVIDER_OR_LICENSE_RATIFICATION` を要求。第三者著作物(SIGNY論考/scan_data収録誌)のOCR・コーパス化は
  ライセンス/複製権の論点が残る。**自所保有物/ライセンス済の確認(owner)を取ってから実取込**とする。
  → owner確認待ちフラグ: `LICENSE_CONFIRM_PENDING`（破壊性GOは下りているが権利確認のみ保留）。
- **2026-06-24 owner: ライセンス確認クリア**「自所物だから進めて」→ `LICENSE_CONFIRM_PENDING` **解除**。
  GO-3/4/5 実取込を承認。head注記: GO-4(OCR)は全量前に**パイロット(1誌数号)で分割境界・OCR品質を検証**してから本走。
