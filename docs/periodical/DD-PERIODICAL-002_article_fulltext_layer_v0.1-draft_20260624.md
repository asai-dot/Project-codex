...
## 3. ソース別取込アダプタ（L0→L1→L2 はソースで分岐）— AI_READY atlas レーンに接地
各ソースは AI_READY_DATA_LOCATOR（atlas）の既知レーン。実パス(`primary_location`/`read_first`/`locator_id`)は
ローカル `docs/alo/AI_READY_DATA_LOCATOR_INDEX_LATEST.tsv` にあり（Box未同期・cloud不可視）。atlasは**参照専用**で
生payload読取/取込は別途 exact owner GO。下表の「atlasレーン/HOLD」は atlas governance(Wave51/57)準拠。
| ソース | atlasレーン | HOLD | 形態 | 必要処理 |
|---|---|---|---|---|
| **DVD由来 生記事/スキャン**(金融・商事判例/金融法務事情 等) | Box `scan_data` | **HIGH-HOLD** | 画像系PDF/TIFF | L1分割→**OCR必須**→L2。ルビ/縦書き/二段組。OCR conf保持 |
| **born-digital PDF** | `scan_data`/別レーン | HOLD | テキスト埋込PDF | L1分割→抽出(OCR不要)→L2 |
| **D1 RAW(文献編)** | D1レーン(Mac-local labeled) | ready(read-only) | 構造化メタ | L4メタ直結。本文無。**記事↔issue_id接合の最短土台(済)** |
| **SIGNY 論考(初出)** | `pacsigny` | **HIGH-HOLD** | 論考メタ | **初出(L4 first_pub)特化**ソース。再録判定の権威 |
| **法学論考** | `legal_thought` | **HIGH-HOLD** | 論考 | L4/L5補助 |
| **TOC/目次** | (要locator確認) | — | 号目次 | seq_in_issue・頁範囲確定、評釈対象判例取得。L1境界＆L5辺 |
→ **HOLD状態が着手順を規定**: ready な D1レーン(L1×L3接合)から着手可。`scan_data`/`pacsigny`/`legal_thought` は
owner GO まで設計のみ。抽象的な優先度判断は不要で、atlasのHOLDが順序を決める。
...
## 7. atlas接地で確定したこと / 残owner判断
**atlas(AI_READY_DATA_LOCATOR)で確定:**
- 生ソースは未定義でなく既知レーン: `scan_data`(DVD/生スキャン), `pacsigny`(SIGNY初出), `legal_thought`(法学論考)。
- これら3レーンは **HIGH-HOLD** → 取込・OCR・初出抽出は owner GO まで設計のみ。D1レーンのみ ready。
- 着手順は atlasのHOLDが規定（L1×L3接合=D1 first → 初出/本文=HOLD解除後）。実務価値の抽象判断は不要。

**残owner判断:**
- `scan_data`/`pacsigny`/`legal_thought` の HOLD解除（取込・OCR・初出抽出のGO、external_share=falseとの整合）。
- DD-PERIODICAL-001(Box)の edge定義との整合（article_published_in/hanrei_published_in 正式スキーマ）。
- alo_works との article_id↔work_id 対応規約の最終化。

**ローカルちゃんへの where-is 依頼(REFERENCE_ONLY・payload読取なし):**
`docs/alo/AI_READY_DATA_LOCATOR_INDEX_LATEST.tsv` + `QUERY_CHEATSHEET` から、雑誌オブジェクト関連レーン
(`pacsigny`/`scan_data`/`legal_thought`/D1文献編/periodical) の `locator_id`/`primary_location`/`read_first`/
`currentness`/`safety_note` だけを抽出して返す。これで各層のソース実体とHOLD状態を確定し、本DDの§3を完成させる。
