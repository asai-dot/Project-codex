...
## 8. Stage2 gold の作り方（owner 判断 2026-06-05・確定）
- **gold は相談1件単位(227)ではなく「事件類型 × 重み付き推薦本」の対応表**にする（粒度＝事件類型≈十数個。現実的）。
- 乗る人手シグナルは2つ: ①事件類型→本の関連度(重み) ②本そのものの良し悪し（文献評価）。
- **これらは独立タスクにしない。** 弁護士が遅かれ早かれ行う**「文献評価」の作業のタイミング**で自然に取得し、その際に
  本 A1 ベースライン（実相談の事件類型分布・現状到達ギャップ・本台紙）を**思い出して土台に使う**。
- したがって今 gold を埋める作業は発生しない。本ファイルと scaffold は「文献評価の時に参照する置き場」。
