...
> **【2026-06-11 追記｜突合は完了済】** books.json(33MB)はリモート転送不可だったが、**同データがSupabase(`biblio.bib_records`)に取り込まれていた**ため、そこで突合を完了：**出典本101冊(ISBN有)中89冊=書式3,762件分をうちが所蔵**。さらに自炊フォルダ(1,148冊)の4点セット監査も実施。結果は `RESULT_holdings_and_scan_audit.md` を参照（本書 §4 の「要worker」前提は解消済み）。
