...
**受容する残差（accepted residual）**
- 分野コードが法学でない学際 project（例: 法と経済・医事法の医学側）は Y-1 で漏れうる。
- 対応: タイトル/キーワード fallback を**サンプルで効果計測**し、費用対効果が出る時だけ追加。出なければ**漏れを明示記録して追わない**（網羅性の線を Owner と合意）。
