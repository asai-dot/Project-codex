...
| `cross_reference_web.py` | all_entries × 権威WEBデータ 照合（OCR引用エラー flag + e-Gov照合） | 同上テストに含む |
...
python3 cross_reference_web.py all_entries.jsonl corrections_web.jsonl   # 既定オフライン
python3 cross_reference_web.py all_entries.jsonl corrections_web.jsonl --verify-egov  # ネット許可時
```
...
- **(A) e-Gov 法令API**（Version 1, GET→XML, https://laws.e-gov.go.jp/docs/law-data-basic/）
  定義文中の「法令名+条番号」引用を検証。`review_queue.json` の **P2_era_concat_digits**
  （例「令167」=施行令167条/政令番号の区切り脱落、129件）を実在条文構造で確定・修正候補化。
- **(B) 法令用語日英標準対訳辞書**（日本法令外国語訳DB, https://www.japaneselawtranslation.go.jp/）
  見出し語そのものを権威ある標準用語リストと突合（`--jlt-terms` で用語jsonl指定）。

生データ非改変。照合結果は `corrections_web.jsonl` 別レイヤに出す。
