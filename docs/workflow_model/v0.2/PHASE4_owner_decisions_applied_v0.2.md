...
- DS-014 GoogleCalendar（相談予定紐付け＝相談記録ソース／**接続あり**＝実査可。「事件メモ」=カレンダー予定に紐づくGoogleドキュメント。owner確認済 2026-06-24）
...
## 3. 残・要確認
- **「事件メモ」**＝カレンダー予定に紐づく Googleドキュメント（owner確認済 2026-06-24。当初「事件ウェブ」と聞き取った点を訂正・W-181で全文反映）。
...
4. ~~「事件ウェブ」定義の最終確認~~ → 完了: 「事件メモ」=カレンダー予定に紐づくGoogleドキュメント（owner確認済 2026-06-24）。
5. PoC1（相談→受任ファネル）の実装着手：相談記録合成（接続済のCalendar+GoogleDocから先行）＋受任=複数シグナル合成の突合ロジック（observed↔declared）。

## 5. W-181 field-level pending 伝播（2026-06-24 追記）

W-180 は決定ログ(D-001..033)・状態機械7本・人間ゲート3マストを確定したが、`ALO_WORKFLOW_CATALOG_v0.2.yaml` に field-level `pending: [owner_grill...]` マーカーが残っていた。W-181 でこれを owner裁定に沿って伝播・確定化した。

- **解消した owner_grill = 23箇所**（全 active `pending:[owner_grill]` がゼロ）。内訳:
  - **confirmed 化(15)**: WF-00/TR-inflow(流入経路 G-12/D-030)・WF-04(PoC1スコープ G-07/D-008)・WF-07(相談票合成 G-14/D-026)・WF-08(失注理由enum G-12/D-030)・WF-11+遷移(受任成立=optional G-06/D-023)・WF-15(WorkItem4必須 G-08/D-011)・WF-16+遷移(内部レビューはマスト外 G-06/D-023)・WF-19(2段クローズ① G-09/D-024)・WF-22+遷移(終結=optional/2段クローズ② G-09/D-024)・ACT-payer(client/payer分離 G-10/D-013)・FE-reconciliation(金銭確定 G-06/G-09)・EXC-urgent_bypass(G-06)・EXC-deadline_missed(G-20/D-033)・EXC-client_unreachable/EXC-close_gate_unmet(G-09/D-024)・EXC-pii_deletion_request(G-03/D-010)。
  - **confirmed_definition 化(KPI 3)**: KPI-P2-05(close gate G-09)・KPI-P2-07(declared/observed G-04)・PoC1/PoC2 scope_status(G-07/G-09/D-008/D-009)。
  - **owner_grill のみ除去し他理由を残置(7、provisional維持)**: WF-03 transition(→staff_interview, G-13は裁定済だが運用粒度は中森聴取)・WF-05(→sf_survey, 2-A Q-062は20問グリル外でowner裁定なし)・WF-17(→sf_survey, 到達正本の現行SFフィールド有無)・WF-18(→sf_survey, Deadline格納先のSF有無)・WF-20+遷移(→sf_survey, 会計系のSF格納先)・WF-21/KPI-P2-04(→box_followup, 返却物/送付パックの追加Box実査)。
- **受任成立(HG-02)・終結(HG-07) を optional に確定**: human_gates では W-180 で既に optional(required:false)。本タスクで WF-11/WF-22 プロセス側の注記・遷移・確認状態も optional/confirmed に整合させた。マスト3点は ①利益相反 ②対外文書最終送付 ③金銭の確定 のみ(G-06/D-023)。
- **「事件ウェブ」→「事件メモ(カレンダー予定に紐づくGoogleドキュメント)」訂正 = 4箇所**: catalog DS-014 note・03_data_sources.csv DS-014・本書 §3・本書 §4。「要最終確認」→「owner確認済(2026-06-24)」。
- 純設計事項で owner裁定が無いものは確定化せず pending理由(owner_grill以外)を残置: 中森聴取(staff_interview)・SF実査(sf_survey)・追加Box実査(box_followup)・外部SE(ext_se)。
- 決定ログ09 は参照のみ（上書きせず）。parse 自己確認: YAML safe_load OK / 全CSV csv.reader OK。
