...
- **サンプル期間・件数**: Calendar = `asai@`(owner業務カレンダー) を `fullText='相談'` で **2025-06-01〜2026-06-24** を走査し1ページ **25件** を取得（nextPageToken残あり＝母集団はこれ以上、構造把握には十分）。Gmail = `subject:(相談)` / `subject:(受任通知)` / `subject:(見積)` を同期間で各 50件上限で走査。Box = `委任契約書` / `委任状` / `見積書` のキーワード検索。
...
採用14件について `event.attachments[].fileUrl` から正規表現 `/document/d/(<doc_id>)/` で doc_id を抽出し、Drive `get_file_metadata` で突合した。**結合検証**: 添付fileUrlのdoc_id 2件（個別相談サンプルA 様式M / 個別相談サンプルB 様式M・当事者名はマスク）を get_file_metadata で照会 → タイトル・所有者・親フォルダが完全解決し、**event.attachments.fileUrl の doc_id = Drive file id が一致**することを再確認（本文非参照）。
...
**補足（自然発生の受任シグナル痕跡）**: (a) 型D 事務所会議議題行の description に「4/3 受任通知発送（8社）」等の進捗語を観測（=Derived補助観測、確定には使わない／W-190 §4 と整合）。(b) Gmail に Box通知（Box由来の自動通知、本文にBoxパス「<案件フォルダ masked>/…請求書（法律相談料）.pdf」）、Chatwork系ToDo通知（「受任案件『<案件名 masked>＿事業整理相談』に関連するToDoが作成されました」）が自然発生で流れている。これらは**将来の自動検出の素材**だが、いずれも相談event と結ぶ共通IDを持たない。
