...
## owner 確認事項（1点）→ **RESOLVED（2026-06-16 浅井）**

- **経済法**: box_prior 2,735 → live 1,954（recall 0.714、removed 781）。
- **owner 判断（2026-06-16）**: D1ロー**民事法セレクションに当該掲載方法（個別法令）が含まれていない**ため
  データが出ない＝**意図的に個別法令を非展開**。これは **D1 側の仕様**であり、現状は契約内容を変更しないため
  **対応不要（どうにもできない前提でよい）**。
- → **residual クローズ**。経済法 removed 781 は live 取りこぼしではなく D1 民事法セレクションの契約スコープ仕様。
  recall 97.29% の残差は box_prior 点描ノイズ＋経済法スコープ仕様で完全に説明済み。
- 残る軽微項目: 改名/移動候補 13 件の目視のみ（精度結論には影響しない）。
