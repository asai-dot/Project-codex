...
当初「大規模前処理が要る」と見たが、§06実測で**唯一の実残課題は短定義 anchor 約580 hub**に縮小した。
P0.5 はこの約580件を triage するだけ。**clean subset は P0.5 を待たず P1 へ進められる**（option 2）。

## 1. スコープ（確定した数字）

| 項目 | hubレベル実数 | 対応 |
|---|---|---|
| 空定義 anchor | **8 hub** | 人手で原典確認（少数）→ see_also or 再OCR |
| 短定義(<8字) anchor | **約580 hub** (507〜581) | 本フェーズの主対象。3分類して処理 |
| 読み欠落 | 0（defmatch救済済1973） | 対応不要（任意でMeCab補完） |
| 非tier1 参照行 | 712（穴ではない） | 対応不要（seed除外が正） |

## 2. 短定義約580の triage 手順（read-only生成→人手判定）
...
- 短定義約580 が ①②③ に分類され、件数が確定（507/581 の差も解消）。
