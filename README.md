...
# ビューワー上のフォーム / API
POST /api/calibrate  {"book_id":"...","source_id":"legal_library","print_page":135,"viewer_page":151}
```

`config/library_sources.json` の有償DLの `url_template` は暫定値。実URLを1本でも取得したら
テンプレートを確定すること（[docs/architecture.md](docs/architecture.md) 参照）。
